...
const {extendRoot} = require("apidoc/lib/core/parsers/api_version");
...
exports.notifySync = function (accessUserId, accessUserType, siteCode, companyCode, type, callback) {
    try {
        ...
        //where.deleted = { $ne: Constant.DELETED.YES }; // đây là cái gif?
        where.isAlive = true;
...
exports.notifyUpgrade = function (accessUserId, accessUserType, id, callback) {
    try {
        ...
        console.log('hello con day la id cua management: ',id);

        if (  Pieces.VariableBaseTypeChecking(id,'number') ){ // (id !== Constant.CONTENT_TYPE_ENUM[0]) && !( Pieces.VariableBaseTypeChecking(id,'string') && Validator.isInt(id) ) &&
        ...
        //where.deleted = { deleted: Constant.DELETED.YES }; //
        where.isAlive = true;
